...
function loadCertificatesData() {
    try {
        ...
        if (fs.existsSync(dataPath)) {
            ...
            console.warn('⚠️  data/certificates-data.json not found, using empty dataset');
        }
    } catch (error) {
...
function enhanceCertificate(cert) {
    ...
    const currentPrice = cert.ask_price || cert.reference_price || 100;
    const effective_annual_yield = cert.annual_coupon_yield 
        ? parseFloat((cert.annual_coupon_yield * (100 / currentPrice)).toFixed(3))
...
function generateScenarioAnalysis(cert, worstUnderlying) {
    ...
    const purchasePrice = cert.ask_price || cert.reference_price || 100;
    ...
    const scenarios = variations.map(varPct => {
        ...
        if (underlyingPrice >= strike) {
            // Above strike: full redemption (100 EUR nominal)
            redemption = 100;
        } else if (underlyingPrice >= barrier) {
            // Between strike and barrier: full redemption (100 EUR)
            redemption = 100;
        } else {
            // Below barrier: proportional loss
            redemption = 100 * (underlyingPrice / strike);
        }

...
class ProductionScraper:
    ...
    def parse_certificate(self, isin, html):
        ...
        def get_underlying_name():
            # Strategy 1: Try to extract from page title/name (most reliable)
            name_elem = soup.find('font', size='+1')
            if name_elem:
                title = name_elem.get_text(strip=True)
                # Common patterns: "CashCollect su ALIBABA", "Phoenix su DAX", etc.
                match = re.search(r'su\s+(.+?)(?:\s+\d|$|\s*-)', title, re.IGNORECASE)
                if match:
                    underlying = match.group(1).strip()
                    # Clean up common suffixes
                    underlying = re.sub(r'\s+(G|GROUP|INDEX|PERFORMANCE).*$', '', underlying, flags=re.IGNORECASE)
                    if len(underlying) > 2:
                        return underlying
            ...
            if section:
                ...
                if parent:
                    table = parent.find('table')
                    if table:
                        # Search for a cell containing "Nome" or "Sottostante" label
                        for row in table.find_all('tr'):
                            cells = row.find_all('td')
                            for i, cell in enumerate(cells):
                                cell_text = cell.get_text(strip=True).lower()
                                # If this cell is a label for underlying name
                                if any(keyword in cell_text for keyword in ['nome', 'sottostante', 'ticker']):
                                    # The value should be in the next cell or same cell
                                    if i + 1 < len(cells):
                                        value = cells[i + 1].get_text(strip=True)
                                        if value and len(value) > 2:
                                            return value
                        
                        # Fallback: Get all text from table and find longest meaningful text
                        all_cells = table.find_all('td')
                        candidates = []
                        for td in all_cells:
                            text = td.get_text(strip=True)
                            # Skip short texts, percentages, dates
                            if text and len(text) > 3 and not re.match(r'^\d+%?$', text) and not re.match(r'\d{2}/\d{2}/\d{4}', text):
